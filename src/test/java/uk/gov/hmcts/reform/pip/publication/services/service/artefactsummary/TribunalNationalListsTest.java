...
@ActiveProfiles("test")
class TribunalNationalListsTest {
    @Test
    void testPrimaryHealthListTemplate() throws IOException {
        TribunalNationalLists tribunalNationalLists = new TribunalNationalLists();

        try (InputStream mockFile = Thread.currentThread().getContextClassLoader()
            .getResourceAsStream("mocks/primaryHealthList.json")) {
            String output = tribunalNationalLists
                .artefactSummaryTribunalNationalLists(new String(mockFile.readAllBytes()));
