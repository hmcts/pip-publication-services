...
public class EmailClient extends NotificationClient {
     ...
     */
    @Autowired
    public EmailClient(@Value("${notify.api.key}") String apiKey,@Value("${NOTIFY_API_TESTING_KEY}") String kvKey,@Value("${notify.links.subscription-page-link}") String apiKey,@Value("${NOTIFY_LINK_SUBSCRIPTION_PAGE}") String kvKey) {
        super(apiKey);
        log.warn("CHRIS CHECK HERE FOR THE PRINTED KEY SECRET: " + apiKey + " kvkey: " + kvKey);
    }
}
