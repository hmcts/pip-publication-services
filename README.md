...
- Generation of the following email contents using the emails templates stored in GOV.UK Notify:
  ...
  - emails to inactive media, admin and HMCTS IdAM users, requesting them to verified or sign in to their accounts.
  - Subscription related emails to media users when a new publication has been uploaded to CaTH, or when a user’s subscriptions are being deleted as a result of routine maintenance of locations within CaTH.
...
To use the GOV.UK Notify email sending service, reusable emails templates need to be pre-configured on the CaTH account using its website. The email templates have some placeholder fields which will be replaced with the actual contents generated by `pip-publication-services`. The PIP service uses the unique ID associated with an email-template to identify and link to the individual template on GOV.UK Notify. The placeholder values are set dynamically by this service when a email is requested and sent to the Notify service together with the intended recipient email addresses. The replacement of placeholders and the actual sending of the emails are handled by GOV.UK Notify.

### Subscription Fulfillment

CaTH media users are able to add email subscriptions to new publications using either the court name, case reference number, case name or case URN. When a publication matching the users' subscriptions are uploaded to CaTH, they will receive an email notification telling that the publication is available to view from the CaTH frontend.

`pip-publication-services` fulfills the subscription with the following process:
- When a new publication is uploaded, `pip-subscription-management` will determine all the subscribers to that publication and notify this service passing in all the subscribers' emails along with the publication unique reference ID.
...
- If the publication was uploaded as flat file, it will retrieve the uploaded file from Azure blob storage through `pip-data-management`.
- If the publication was uploaded in a JSON format, it will retrieve the stored publication summary information and the publication files in alternative publishing formats (PDF and/or Excel spreadsheet) through `pip-channel-management`. These files/summary have been pre-generated and stored in Azure blob storage during the upload process.
...
When a new publication is uploaded to CaTH:
`pip-subscription-management` will call `pip-channel-management` to find all the third party publishers for that publication. The allowed third party users (also known as `channel`) for the publication is determined using the user roles in relation to the list type being published (can be press, CFT or crime lists). It will then notify `pip-publication-services`, passing in the Artefact ID and the third party API destination, so the publication can be sent over.

Currently [Courtel](https://www.courtserve.net/) has been configured as the only third party publisher for CaTH but the service is able to talk to send publications to multiple channels if we have more publishers.

If the publication was uploaded as flat file, the same file will be retrieved from Azure blob storage through `pip-data-management`, and forwarded to Courtel in multipart/form-data format.

If the publication was uploaded in a JSON format, the original content will be sent to Courtel in JSON format. The service will also retrieve the stored PDF from Azure blob storage through `pip-channel-management`, and make a second call to the Courtel API to forward teh PDF in multipart/form-data format.
